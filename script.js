...
// PERSISTENCE (Specific to Motorbikes App)
let lifetimeScore = parseInt(localStorage.getItem('motorbikesScore')) || 0;
let completedLessons = JSON.parse(localStorage.getItem('completedMotorbikesLessons')) || [];
if(ptsVal) ptsVal.innerText = lifetimeScore;

let wordBucket = []; let currentQ = 0; let attempts = 0; let totalScore = 0; let firstCard = null;

// STATIONS (Matching the Colab Audio Output)
const stations = [
    {file:"01_Aprilia.mp3", title:"Aprilia"},
    {file:"02_BMWMotorrad.mp3", title:"BMW Motorrad"},
    {file:"03_BSA.mp3", title:"BSA"},
    {file:"04_Ducati.mp3", title:"Ducati"},
    {file:"05_HarleyDavidson.mp3", title:"Harley-Davidson"},
    {file:"06_Honda.mp3", title:"Honda"},
    {file:"07_Husqvarna.mp3", title:"Husqvarna"},
    {file:"08_IndianMotorcycle.mp3", title:"Indian Motorcycle"},
    {file:"09_Kawasaki.mp3", title:"Kawasaki"},
    {file:"10_KTM.mp3", title:"KTM"},
    {file:"11_MotoGuzzi.mp3", title:"Moto Guzzi"},
    {file:"12_MVAgusta.mp3", title:"MV Agusta"},
    {file:"13_Norton.mp3", title:"Norton"},
    {file:"14_RoyalEnfield.mp3", title:"Royal Enfield"},
    {file:"15_Suzuki.mp3", title:"Suzuki"},
    {file:"16_Triumph.mp3", title:"Triumph"},
    {file:"17_Vespa.mp3", title:"Vespa"},
    {file:"18_Victory.mp3", title:"Victory"},
    {file:"19_Vincent.mp3", title:"Vincent"},
    {file:"20_Yamaha.mp3", title:"Yamaha"},
    {file:"21_ZeroMotorcycles.mp3", title:"Zero Motorcycles"}
];
...
function runQuiz(lesson) {
    ...
    document.getElementById('btn-hear-q').onclick = () => {
        ...
        if (voices.length > 0) {
            // Using logic to switch voices automatically
            if (storyNum % 2 !== 0) {
...
function finishQuiz() {
    lifetimeScore += totalScore; localStorage.setItem('motorbikesScore', lifetimeScore);
    const fn = decodeURIComponent(audio.src.split('/').pop());
    if(!completedLessons.includes(fn)) {
        completedLessons.push(fn); localStorage.setItem('completedMotorbikesLessons', JSON.stringify(completedLessons));
    }
